...
class VotePostgresRepository implements IVoteRepository {
    ...
    public async create(data: ICreateVoteDTO): Promise<IVoteEntity> {
        ...
        const voteCreated = this._repository.create({ campaign_id, emotion_id, user_id: user_token_id });
    ...
    public async list(): Promise<IVoteEntity[]> {
        return await this._repository.find(/*{ relations: ['campaign', 'emotion', 'user'] }*/);
    }
}
